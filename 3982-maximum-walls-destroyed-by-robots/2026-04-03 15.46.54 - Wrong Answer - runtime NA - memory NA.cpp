...
public:
    int maxWalls(vector<int>& robots, vector<int>& distance, vector<int>& walls) {
        ...
        dp[0][1] = countWalls(rob[0].first, rob[0].first);

        for (int i = 0; i < n - 1; i++) {
            int pos1 = rob[i].first,   d1 = rob[i].second;
            int pos2 = rob[i+1].first, d2 = rob[i+1].second;

            int A = countWalls(pos1 + 1, min(pos1 + d1, pos2 - 1));
            int B = countWalls(max(pos2 - d2, pos1 + 1), pos2 - 1);
            int ownNext = countWalls(pos2, pos2);

            dp[i+1][0] = ownNext + max(dp[i][0] + B,
                                       dp[i][1] + A + B);

            dp[i+1][1] = ownNext + max(dp[i][0],
                                       dp[i][1] + A);
        }

        int posLast = rob[n-1].first, dLast = rob[n-1].second;
        int rightmost = countWalls(posLast + 1, posLast + dLast);

        return max(dp[n-1][0], dp[n-1][1] + rightmost);
    }
};